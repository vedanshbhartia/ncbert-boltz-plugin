...
PARALLEL="${PARALLEL:-3}"
NSTRUCT="${NSTRUCT:-1}"
RUN_TAG="${RUN_TAG:-1a85_relax_variant_pilot_$(date +%Y%m%d_%H%M%S)}"
...
VARIANTS=(
  "baseline:relax_script_thread_1a85_chainA.xml"
  "rigid_jump:relax_script_thread_1a85_chainA_rigid_jump.xml"
  "staged_backbone:relax_script_thread_1a85_chainA_staged.xml"
  "staged_backbone_rigid_jump:relax_script_thread_1a85_chainA_staged_rigid_jump.xml"
)
